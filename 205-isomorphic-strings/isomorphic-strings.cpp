...
public:
    bool isIsomorphic(string s, string t) {
        ...
        for (int i = 0; i < s.size(); ++i) {
            ...
            // Check if there's a mapping for charS in mapS2T and if it maps to
            // the same character in t
            if (mapS2T.find(charS) != mapS2T.end()) {
                if (mapS2T[charS] != charT) {
                    return false;
                }
            } else { // If no mapping exists, check if charT is already mapped
                     // to some other character in mapT2S
